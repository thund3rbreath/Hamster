...
class HttpRequest:
    def __init__(
        ...
        if "windows" in self.user_agent.lower():
            self.log.warning(
                "🟡 <yellow>Windows User Agent detected, For safety please use mobile user-agent</yellow>"
            )
    ...
    def get(
        ...
        try:
            ...
            if response.status_code != valid_response_code:
                self.log.error(
                    f"🔴 <red> GET Request Error: <yellow>{url}</yellow> Response code: {response.status_code}</red>"
                )
            ...
            if retries > 0:
                self.log.info(f"🟡 <yellow> Unable to send request, retrying...</yellow>")
                time.sleep(0.5)
                return self.get(
                    url,
                    headers,
            ...
            self.log.error(f"🔴 <red> GET Request Error: <yellow>{url}</yellow> {e}</red>")
            return (None, None) if return_headers else None
    ...
    def post(
        ...
        try:
            ...
            if response.status_code != valid_response_code:
                print(response.text)
                self.log.error(
                    f"🔴 <red> POST Request Error: <yellow>{url}</yellow> Response code: {response.status_code}</red>"
                )
                return (None, None) if return_headers else None
            ...
            self.log.error(f"🔴 <red> POST Request Error: <yellow>{url}</yellow> {e}</red>")
            return (None, None) if return_headers else None
    ...
    def options(self, url, method, headers=None, valid_response_code=204, retries=3):
        try:
            ...
            if response.status_code != valid_response_code:
                self.log.error(
                    f"🔴 <red> OPTIONS Request Error: <yellow>{url}</yellow> Response code: {response.status_code}</red>"
                )
                return None
            ...
            if retries > 0:
                self.log.info(f"🟡 <yellow> Unable to send option request, retrying...</yellow>")
                time.sleep(0.5)
                return self.options(
